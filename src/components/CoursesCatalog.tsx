...
export const CoursesCatalog: React.FC<CoursesCatalogProps> = ({
  ...
  return (
    <div className="flex flex-col gap-8 pb-10">
          ...
          >
            <div>
              ...
              <div className="text-[11px] font-bold uppercase tracking-wider text-emerald-400 mb-1 font-mono">
                Manual pag. 27–30 • Unitatea 2
              </div>

              <h3 className="text-xl font-black text-white font-heading mb-1 group-hover:text-emerald-300 transition-colors">
                Misiunea Arborele Secret de Fișiere
              </h3>
              <div className="text-xs font-semibold text-slate-300 mb-3">
                Foldere, Căutare Wildcard, Comenzi Rapide & Recycle Bin
              </div>

              <p className="text-xs sm:text-sm text-slate-400 leading-relaxed mb-4">
                Construiește structura ierarhică de directoare din manual, găsește fișierele secrete cu masca (*.docx), mută documente cu comenzi rapide de tastatură, gestionează proprietățile fișierelor și restaurează datele din Recycle Bin!
              </p>
              ...
              <div className="flex flex-wrap gap-2 mb-4 text-[11px] font-mono">
                ...
                <span className="px-2.5 py-1 rounded-lg bg-slate-950/60 border border-slate-800 text-emerald-300 flex items-center gap-1">
                  <Award className="w-3 h-3 text-emerald-400" /> Diplomă Arhitect Fișiere
                </span>
              </div>
            </div>
      ...
      {/* Teacher Link Banner */}
      {onOpenTeacherPortal && (
        <div className="bg-slate-900/80 border border-amber-500/30 rounded-2xl p-4 sm:p-5 flex flex-col sm:flex-row items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl bg-amber-500/20 text-amber-400 flex items-center justify-center font-bold text-lg">
              👨‍🏫
            </div>
            <div>
              <div className="text-xs font-black uppercase tracking-wider text-amber-400 font-mono">
                Acces Profesori & Notare
              </div>
              <div className="text-sm font-bold text-slate-200">
                Verifică catalogul de note, timpii și clasamentul elevilor în timp real
              </div>
            </div>
          </div>
          ...
          <button
            ...
            className="px-4 py-2 rounded-xl bg-amber-500/20 hover:bg-amber-500/30 text-amber-300 border border-amber-500/40 text-xs font-bold transition shrink-0 cursor-pointer"
          >
            Deschide Portalul Profesorului
          </button>
        </div>
      )}
      ...
      <MissionGuardModal
        ...
        activeScore={activeScore(activeMissionLevel)}
...
function activeScore(level: number): number {
  if (level <= 1) return 0;
